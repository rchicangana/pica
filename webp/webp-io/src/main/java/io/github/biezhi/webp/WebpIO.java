...
public class WebpIO {
    ...
    private static final String CWEBP_TMP_DIR = "C:\\windows\\temp\\cwebp";
    ...
    static {
        ...
        if ("true".equals(devMode)) {
            ...
            if (!tmp.exists()) {
                ...
                InputStream dwebp = null;
                InputStream cwebp = null;
                InputStream gif2webp = null;
            try {
                dwebp = new FileInputStream(new File("C:\\windows\\temp\\"+webpPath + "\\dwebp" + extension));
                cwebp =  new FileInputStream(new File("C:\\windows\\temp\\" + webpPath + "\\cwebp" + extension));
                gif2webp =  new FileInputStream(new File("C:\\windows\\temp\\" + webpPath + "\\gif2webp" + extension));
            } catch (FileNotFoundException ex) {
                Logger.getLogger(WebpIO.class.getName()).log(Level.SEVERE, null, ex);
            }
    ...
    public static void toNormalImage(File src, File dest) {
        String command = CMD_DIR + (dest.getName().endsWith(".gif") ? "\\gif2webp" : "\\dwebp.exe ") + src.getPath() + " -o " + dest.getPath();
    ...
    public static void toWEBP(File src, File dest) {
        try {
            String command = CMD_DIR + (src.getName().endsWith(".gif") ? "\\gif2webp " : "\\cwebp.exe ") + src.getPath() + " -q 0 -o " + dest.getPath();
            System.out.println("Execute: " + command);
            String output = executeCommand(command);
            if (!"".equals(output)) {
                System.out.println("Output: " + output);
